...
class VPAnalysisAPI:
    ...
    def __init__(
        ...
        if api_key is not None:
            ...
            if not self.api_key:
                raise AuthenticationError("No API key provided. Set VP_ANALYSIS_API_KEY environment variable or pass api_key parameter.")
    ...
    def get_security_factors(
        ...
        try:
            ...
            df = df.pivot(
                index="dt",
                columns=["stock_code", "factor_identifier"],
                values="value"
            )
            return df
            ...
            raise APIRequestError(f"Unexpected error while getting security factors: {str(e)}") from e
